...
#include "../../src/ngx_http_sayplease_module.h"
...
void test_is_blocked_url(void) {
    ...
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_is_blocked_url(&clean_url));
    TEST_ASSERT_EQUAL(NGX_ERROR, ngx_http_sayplease_is_blocked_url(&blocked_url));
}
...
void test_load_robots(void) {
    ...
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_load_robots(&robots_path));
}
...
void test_generate_content(void) {
    ...
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_generate_content(&url, &content, &content_len));
    TEST_ASSERT_NOT_NULL(content);
    TEST_ASSERT_TRUE(content_len > 0);
    TEST_ASSERT_TRUE(strstr((char *)content, "SayPlease") != NULL);
}
...
void test_cache_operations(void) {
    ...
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_init_cache());
    
    // Test lookup for existing entry
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_cache_lookup(test_fingerprint, &found));
    TEST_ASSERT_EQUAL(1, found);
    
    // Test lookup for non-existent entry
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_cache_lookup(nonexistent, &found));
    TEST_ASSERT_EQUAL(0, found);
    
    // Test insertion
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_cache_insert(nonexistent));
}
...
void test_database_operations(void) {
    ...
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_init_db(&db_path));
    TEST_ASSERT_EQUAL(NGX_OK, ngx_http_sayplease_log_request(&url));
}
